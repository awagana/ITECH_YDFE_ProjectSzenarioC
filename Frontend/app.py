...
def get_questions():
    try:
        ...
        for row in rows:
            tag = row['tag']
            if tag not in quiz_data: quiz_data[tag] = []
...
def login():
    data = request.json
    user_db = get_db_connection().execute("SELECT * FROM users WHERE username = ?", (data.get('username'),)).fetchone()
...
def save_score():
    ...
    try:
        ...
        existing = conn.execute("SELECT score FROM scores WHERE username = ? AND category = ?", (username, category)).fetchone()
...
def get_leaderboard(category):
    try:
        conn = get_db_connection()
        rows = conn.execute("SELECT username, score FROM scores WHERE category = ? ORDER BY score DESC LIMIT 10", (category,)).fetchall()
        conn.close()
